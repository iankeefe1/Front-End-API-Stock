...
export function StockInAdd() {
  ...
  // ------------------------------------------------------------------------------------------------------------------------------------------------------
  // Route params
  const { transactionid, pagestate, approvalid } = useSearch({
    from: '/_authenticated/Stock/StockIn/Add/',
  })
  ...
  // 👇 final condition
  const shouldDisable = isReadOnly && (!!transactionid || !!approvalid)
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
        const res = await fetch(
          `${API_BASE_URL}/Product/GetProductById?transactionid=${transactionid}`
        )
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <DatabaseDropdown
                    apiUrl={`${API_BASE_URL}/Product/GetProductCompleted`}
                    labelKey="productName"
                    valueKey="productID"
                    value={productId?.toString()}
                    onValueChange={(val, item) => {
                      setProductId(Number(val))
                      setCatalogueName(item?.productName ?? "")
                    }}
                  />
